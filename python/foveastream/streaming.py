...
from . import Falloff, FoveationConfig, Roi, context_and_roi_views, foveate, qp_delta_map, quality_map
...
class MultiRoiTracker:
    ...
    def _correct(self, track: RoiTrack, proposal: RoiProposal, dt: float) -> None:
        ...
        beta = float(np.clip(cfg.velocity_smoothing, 0, 1)); track.velocity = track.velocity * (1-beta) + obs * beta
        alpha = float(np.clip(cfg.smoothing, 0, 1)); blend = lambda a, b: a * (1-alpha) + b * alpha
        track.roi = Roi(blend(old.x,z.x), blend(old.y,z.y), blend(old.w,z.w), blend(old.h,z.h), proposal.confidence).clipped()
        track.confidence = float(np.clip(.35 * track.confidence + .65 * proposal.confidence, 0, 1)); track.roi.confidence = track.confidence
        track.priority = max(track.priority, float(proposal.priority)); track.source = proposal.source; track.stale_s = 0.0

    def update(self, proposals: Sequence[RoiProposal] = (), dt_s: float = 0.0) -> list[Roi]:
        cfg = self.config; dt = float(np.clip(dt_s, 0, 1)); self._predict(dt); used: set[int] = set()
        for p in self._dedupe(proposals):
            r = p.roi.clipped()
            if r.w <= 0 or r.h <= 0 or p.confidence <= 0: continue
            best_idx, best_score = None, -1e9
            for i, track in enumerate(self.tracks):
                if i in used: continue
                if p.track_hint is not None and p.track_hint == track.id:
                    best_idx, best_score = i, 10.0; break
                overlap = roi_iou(track.roi, r); dist = _center_distance(track.roi, r)
                if overlap < cfg.association_iou and dist > cfg.association_center_distance: continue
                compatibility = overlap * 2 + (1-min(1.0, dist)) + .25 * p.score
                if compatibility > best_score: best_idx, best_score = i, compatibility
            if best_idx is not None:
                self._correct(self.tracks[best_idx], p, max(dt, 1e-4)); used.add(best_idx)
            elif len(self.tracks) < max(1, cfg.max_tracks):
                self.tracks.append(RoiTrack(
                    id=self._next_id, roi=Roi(r.x, r.y, r.w, r.h, p.confidence).clipped(), velocity=np.zeros(4, np.float32),
                    confidence=float(np.clip(p.confidence, 0, 1)), priority=max(0.0, float(p.priority)), source=p.source,
                )); used.add(len(self.tracks)-1); self._next_id += 1
        self.tracks = [t for t in self.tracks if t.stale_s <= cfg.max_stale_s and t.confidence >= cfg.min_confidence]
        self.tracks.sort(key=lambda t: t.score, reverse=True); del self.tracks[max(1, cfg.max_tracks):]
...
class ClassAgnosticMotionRoiDetector:
    ...
    def __init__(self, config: MotionDetectorConfig | None = None):
        if cv2 is None: raise RuntimeError("ClassAgnosticMotionRoiDetector requires opencv-python; install foveastream[video]")
        self.config = config or MotionDetectorConfig(); self._prev: np.ndarray | None = None

    def reset(self) -> None: self._prev = None

    def propose(self, frame_rgb: np.ndarray) -> list[RoiProposal]:
        h0, w0 = frame_rgb.shape[:2]; w = max(32, int(self.config.analysis_width)); h = max(1, round(h0*w/w0))
        ...
        p0 = cv2.goodFeaturesToTrack(prev, maxCorners=220, qualityLevel=.015, minDistance=7, blockSize=5); transform = None
        if p0 is not None and len(p0) >= 6:
            ...
            if p1 is not None:
                ...
                if len(a) >= 6: transform, _ = cv2.estimateAffinePartial2D(a,b,method=cv2.RANSAC,ransacReprojThreshold=2.0,maxIters=300)
        ...
        mask = (mag > threshold).astype(np.uint8)*255; mask = cv2.morphologyEx(mask,cv2.MORPH_OPEN,np.ones((3,3),np.uint8)); mask = cv2.morphologyEx(mask,cv2.MORPH_CLOSE,np.ones((7,7),np.uint8))
        n, labels, stats, _ = cv2.connectedComponentsWithStats(mask,8); proposals: list[RoiProposal] = []; frame_area = h*w
        for i in range(1,n):
            x,y,bw,bh,area = stats[i]; ratio = area/max(1,frame_area)
            if ratio < self.config.min_area_ratio or ratio > self.config.max_area_ratio: continue
            pad_x, pad_y = bw*self.config.padding, bh*self.config.padding
            rx, ry = (x-pad_x)/w, (y-pad_y)/h; rw, rh = (bw+2*pad_x)/w, (bh+2*pad_y)/h
            strength = float(np.mean(mag[labels==i])); compactness = area/max(1,bw*bh)
        ...
        proposals.sort(key=lambda p: p.score, reverse=True); return proposals[:max(1,self.config.max_proposals)]
...
class AdaptiveScheduler:
    def __init__(self, config: SchedulerConfig | None = None): self.config = config or SchedulerConfig(); self.time_since_send_s = float("inf")
    def reset(self) -> None: self.time_since_send_s = float("inf")
    def step(self, dt_s: float, s: InnovationSignals) -> SendDecision:
        c = self.config; self.time_since_send_s = min(1e6, self.time_since_send_s + max(0.0, dt_s)); age = np.clip(self.time_since_send_s/max(c.max_refresh_interval_s,1e-6),0,1)
        score = (c.pose_weight*np.clip(s.pose_novelty,0,1) + c.roi_weight*np.clip(s.roi_prediction_error,0,1) + c.flow_weight*np.clip(s.residual_motion,0,1)
            + c.scene_weight*np.clip(s.scene_change,0,1) + c.semantic_weight*np.clip(s.semantic_uncertainty,0,1) + c.age_weight*age)
        ...
        if send and not s.hard_trigger and self.time_since_send_s < c.min_interval_s: send,reason=False,"suppressed-min-interval"
...
class StreamRuntimeConfig:
    foveation: FoveationConfig = field(default_factory=lambda: FoveationConfig(peripheral_scale=1/8))
    tracker: RoiTrackerConfig = field(default_factory=RoiTrackerConfig)
    scheduler: SchedulerConfig = field(default_factory=SchedulerConfig)
    context_scale: float = 0.25
    quality_floor: float = 0.04
    ...
    @classmethod
    def preset(cls, name: str, *, auto_motion_proposals: bool = False) -> "StreamRuntimeConfig":
        key = name.lower()
        cfg = cls(auto_motion_proposals=auto_motion_proposals)
        if key == 'balanced':
            return cfg
        if key == 'aggressive':
            cfg.foveation.peripheral_scale = 1/16
            cfg.foveation.falloff_x = cfg.foveation.falloff_y = .18
            cfg.foveation.falloff = Falloff.GAUSSIAN
            cfg.tracker.max_tracks = 6; cfg.tracker.pixel_budget_fraction = .22
            cfg.context_scale = .15; cfg.quality_floor = .01; cfg.uncertainty_floor = .02
            return cfg
        if key == 'extreme':
            cfg.foveation.peripheral_scale = 1/24
            cfg.foveation.falloff_x = cfg.foveation.falloff_y = .12
            cfg.foveation.falloff = Falloff.GAUSSIAN
            cfg.tracker.max_tracks = 4; cfg.tracker.pixel_budget_fraction = .15
            cfg.context_scale = .10; cfg.quality_floor = 0.0; cfg.uncertainty_floor = .01
            return cfg
        raise ValueError("preset must be one of: balanced, aggressive, extreme")
...
class ProcessResult:
    ...
    @property
    def context(self) -> np.ndarray | None: return self.views[0] if self.views else None
    @property
    def roi_views(self) -> list[np.ndarray]: return self.views[1:] if len(self.views)>1 else []
...
class CallbackSink:
    def __init__(self, callback: Callable[[ProcessResult], None], *, only_when_send: bool = False): self.callback = callback; self.only_when_send = only_when_send
    def consume(self, result: ProcessResult) -> None:
        if not self.only_when_send or result.decision.send: self.callback(result)
...
class FoveaStreamRuntime:
    ...
    def __init__(self, config: StreamRuntimeConfig | None = None):
        self.config = config or StreamRuntimeConfig.preset('balanced')
        self.tracker = MultiRoiTracker(self.config.tracker); self.scheduler = AdaptiveScheduler(self.config.scheduler)
        self.motion = ClassAgnosticMotionRoiDetector() if self.config.auto_motion_proposals else None; self._last_timestamp_s: float | None = None

    def reset(self) -> None:
        self.tracker.reset(); self.scheduler.reset(); self._last_timestamp_s = None
        if self.motion is not None: self.motion.reset()

    def process(self, frame_rgb: np.ndarray, timestamp_s: float, *, proposals: Sequence[RoiProposal] = (), points: Sequence[tuple[float,float]] = (), signals: InnovationSignals | None = None) -> ProcessResult:
        frame = np.ascontiguousarray(frame_rgb, dtype=np.uint8)
        if frame.ndim != 3 or frame.shape[2] != 3: raise ValueError("frame_rgb must be HxWx3 uint8 RGB")
        timestamp_s = float(timestamp_s)
        if not np.isfinite(timestamp_s): raise ValueError("timestamp_s must be finite")
        if self._last_timestamp_s is not None and timestamp_s + 1e-9 < self._last_timestamp_s: raise ValueError("timestamps must be monotonic")
        dt = 0.0 if self._last_timestamp_s is None else max(0.0, timestamp_s-self._last_timestamp_s); self._last_timestamp_s = timestamp_s

        all_proposals = list(proposals)
        if self.motion is not None: all_proposals.extend(self.motion.propose(frame))
        rois = self.tracker.update(all_proposals, dt)
        q = quality_map(frame.shape, points=points, rois=rois, config=self.config.foveation)
        mean_conf = float(np.mean([t.confidence for t in self.tracker.tracks])) if self.tracker.tracks else (1.0 if points else 0.0)
        floor = float(np.clip(self.config.quality_floor + self.config.uncertainty_floor*(1-mean_conf),0,1)); q = np.maximum(q,floor).astype(np.float32)

        sig = signals or InnovationSignals()
        if sig.uncertainty_radius <= 0 and self.tracker.tracks: sig.uncertainty_radius = max(t.stale_s*self.config.tracker.uncertainty_growth_per_s for t in self.tracker.tracks)
        if sig.semantic_uncertainty <= 0 and self.tracker.tracks: sig.semantic_uncertainty = float(np.clip(1-mean_conf,0,1))
...
def run_stream(frames: Iterable[tuple[float,np.ndarray]], runtime: FoveaStreamRuntime, sink: StreamSink) -> None:
    """Generic pull-source bridge. Provider/camera adapters only need to yield (timestamp_s, RGB frame)."""
    for timestamp_s, frame_rgb in frames: runtime.push(frame_rgb,timestamp_s,sink)
